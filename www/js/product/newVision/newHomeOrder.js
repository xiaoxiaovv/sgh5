 ...
 * 刘成杰 2017-07-16.
 */
APP.controller('newHomeOrderController', ['$ionicPlatform','$ionicHistory','CLASSIFYMESSAGECRNTERService','UrlService','NewPersonService', 'trueAuthenticationService', '$localstorage', 'GoodsSearchService', 'RegisterService', '$ionicModal', 'ShopService', '$q', 'GoodsService', 'CalculateStrLength', '$interval', '$http', '$scope', '$stateParams', '$ionicSlideBoxDelegate',
  '$state', 'UserService', '$timeout', '$rootScope', '$ionicScrollDelegate', 'CommonAddressService', 'BannerThemeService', 'PopupService', 'newHomeLifeService', 'ProductDetailService',
  function ($ionicPlatform,$ionicHistory,CLASSIFYMESSAGECRNTERService,UrlService,NewPersonService ,trueAuthenticationService, $localstorage, GoodsSearchService, RegisterService, $ionicModal, ShopService, $q, GoodsService, CalculateStrLength, $interval, $http, $scope, $stateParams, $ionicSlideBoxDelegate, $state, UserService, $timeout, $rootScope, $ionicScrollDelegate, CommonAddressService, BannerThemeService, PopupService, newHomeLifeService, ProductDetailService) {
    ...
    $scope.urls = "http://m.ehaier.com/v3/sgcommunity/diy/login/request.ajax"+"?flag="+$localstorage.get('sg_login_token_secret').substring(6);
    $scope.flagNum = false;
    $scope.flashHour = '';
    $scope.flashText='';
    ...
    $scope.init = function () {
        $('.orderContent').html('<iframe style="width:100%;height:100%;" src='+$scope.urls+'></iframe>')
          ...
          newHomeLifeService.getLifeDetail($scope.streetId,$scope.areaId,$scope.provinceId,$scope.cityId)
            .success(function(res){
                ...
                for(var i=0;i<res.data.length;i++){
                    if(res.data[i].id==0){
                        $scope.relaxList=res.data[i].products;
                    }else if(res.data[i].id==1){
                        $scope.onlyList=res.data[i].products;
                    }
                }
    ...
    $scope.goToManageMoney = function () {
      if ($rootScope.isWdHost == 1) {
        $state.go('financialMoney', {whereId: 'newHomeOrder'});
