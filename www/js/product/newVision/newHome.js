...
APP.controller('newHomeController', ['$ionicPlatform','$ionicHistory','CLASSIFYMESSAGECRNTERService','UrlService','NewPersonService', 'trueAuthenticationService', '$localstorage', 'GoodsSearchService', 'RegisterService', '$ionicModal', 'ShopService', '$q', 'GoodsService', 'CalculateStrLength', '$interval', '$http', '$scope', '$stateParams', '$ionicSlideBoxDelegate',
  '$state', 'UserService', '$timeout', '$rootScope', '$ionicScrollDelegate', 'CommonAddressService', 'BannerThemeService', 'PopupService', 'newHomeService', 'ProductDetailService','newHomeMakeService',
  function ($ionicPlatform,$ionicHistory,CLASSIFYMESSAGECRNTERService,UrlService,NewPersonService ,trueAuthenticationService, $localstorage, GoodsSearchService, RegisterService, $ionicModal, ShopService, $q, GoodsService, CalculateStrLength, $interval, $http, $scope, $stateParams, $ionicSlideBoxDelegate, $state, UserService, $timeout, $rootScope, $ionicScrollDelegate, CommonAddressService, BannerThemeService, PopupService, newHomeService, ProductDetailService,newHomeMakeService) {
    ...
    $scope.chooseType = function (index, event) {
      ...
      //new
      $scope.gotoFullhouseSolution=function(id){
          $state.go('fullHouseSolution',{soluteId:id})
      }
      $scope.loadMore=function(){
        $scope.pageindex1++
        newHomeMakeService.getTopic(1,1,$scope.pageindex1,6)
        .success(function(res){
            console.log(res.data)
            $scope.smartLife.storys=$scope.smartLife.storys.concat(res.data.storys);
    ...
    $scope.init = function () {
       ...
       newHomeService.getSmartHome()
        .success(function(res){
        console.log(res);
        $scope.programNav=res.data.program.programs;
        $scope.programtitle = res.data.program.title;
        $scope.recommend=res.data.recommend.recommends;
        $scope.recommendtitle = res.data.recommend.title;
        $scope.recommendId = res.data.recommend.id;
          ...
          var swiper = new Swiper('.swiper-container',{
            pagination: {
                ...
                renderBullet: function (index, className) {
                  console.log(index)
                  if(index==0){
                    return '<span class="' + className + ' loopNav"><i>'+$scope.scenes[0].name+'</i></span>';
                  }else if(index==1){
                    return '<span class="' + className + ' loopNav"><i>'+$scope.scenes[1].name+'</i></span>';
                  }else if(index==2){
                    return '<span class="' + className + ' loopNav"><i>'+$scope.scenes[2].name+'</i></span>';
                  }else if(index==3){
                    return '<span class="' + className + ' loopNav lastNav"><i>'+$scope.scenes[3].name+'</i></span>';
                  }
        ...
        newHomeMakeService.getTopic(0,1,1,2)
        .success(function(res){
            console.log(res.data)
            $scope.videoFeel=res.data
        })
        $scope.pageindex1=1;
        newHomeMakeService.getTopic(1,1,$scope.pageindex1,6)
        .success(function(res){
            console.log(res.data)
            $scope.smartLife=res.data;
            if(res.data.storys.length<6){
              $scope.hasmore=false;
              $scope.bottomIF=true;
            }else{
 ...
 //                      newHomeService.getRecently($scope.cityIds,1,$scope.lat,$scope.lons,1)
 //                          .success(function(res){
 //                            console.log(res);
 //                            if(res.success){
 //                              if(res.data){
 ...
 //            newHomeService.getRecently($scope.cityIds,1,36.108883,120.457091,1)
 //            .success(function(res){
 //              console.log(res);
 //              if(res.success){
 //                if(res.data){
 ...
 //          }
    ...
    //   $interval.cancel($scope.timer);
    //   $interval.cancel($scope.timer1);
    //   newHomeService.getFlashSales($scope.provinceId, $scope.cityId, $scope.areaId, $scope.streetId)
    //     .success(function (res) {
    ...
    //无信息弹窗
    $scope.noMessage = function(){
      $scope.isNoMessage=false;
    }
    //跳转活动页-新手礼
    $scope.goHD = function (a,b) {
      console.log(b);
      if (a == '1') {
        // alert( '单品页'+b);

        if(b.storeId == ''){
          // console.log(b.storeId)
          $state.go('productDetail',{fromType:b.fromType,fromUrl:b.fromUrl,o2oType:b.o2oType,productId:b.productId,storeId:$scope.storeId});
        }else{
          $state.go('productDetail',{fromType:b.fromType,fromUrl:b.fromUrl,o2oType:b.o2oType,productId:b.productId,storeId:b.storeId});
        }
        ...
        if(!b.id){
          ...
          $state.go('couponsDetail',{cId:b.id,userID:$scope.storeId,type:2});
        }
      } else if (a == '3') {
        // alert( '游戏页'+b);
        $state.go('game',{gameId:b.gameId});
      } else if (a == '4') {
        // alert( '秒杀活动页'+b);
        if (b.activityType == '日常活动') {
          BannerThemeService.getBannerTheme(b.bannerId)
            .success(function (response) {
              console.log(response);
              $state.go('bannerDaily', {bannerId: b.bannerId, layout: response.data.layout});
            });
        }else if(b.activityType == '主题活动') {
          $state.go('bannerTheme', {bannerId: b.bannerId});
        }
      } else if (a == '5') {
        // alert( '自定义类型页'+b);
          $state.go('customizehd',{url:b.url});
      }
    };
    ...
    $scope.goToManageMoney = function () {
      if ($rootScope.isWdHost == 1) {
        $state.go('financialMoney', {whereId: 'newHome'});
...
APP.service('newHomeService', ['$http', 'UrlService', function ($http, UrlService) {
