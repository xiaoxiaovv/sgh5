APP.controller('newAuthenticationHomeV2', ['GetStatisticInfoService','IMService','ADDADDRESSService','AccountMessageService','$localstorage','$scope', '$rootScope', '$state', 'OpenStoreService', 'MyStoreService', 'CommonAddressService', 'PopupService', 'LoginService',
  'UserService', 'ShopService','$stateParams','$ionicLoading','$timeout','$ionicPopup', '$ionicScrollDelegate', '$ionicModal', '$http',
  function (GetStatisticInfoService,IMService,ADDADDRESSService,AccountMessageService,$localstorage,$scope, $rootScope, $state, OpenStoreService,MyStoreService,CommonAddressService, PopupService, LoginService, UserService, ShopService,$stateParams,$ionicLoading,$timeout,$ionicPopup,$ionicScrollDelegate,$ionicModal,$http) {
    var parentInfoView = '<div style="background:#ffffff;width:304px;border-radius:8px;">\
                            <div style="padding:20px 10px;">\
                              <div>\
                                <h6 align="center" style="font-size:16px;line-height: 16px;color:#333333;">请确认您的推荐人信息</h6>\
                                <h2 align="center" class="promotion-code" style="font-size:22px;line-height:22px;"></h2>\
                              </div>\
                              <div style="padding:10px;margin:-10px auto;">\
                                <div style="margin: 0 auto;width: 0px;height: 0px;border-width: 10px;border-style: solid;border-color: #ffffff #ffffff #EEEEEE #ffffff;"></div>\
                                <div style="background:#EEEEEE;padding:20px;border-radius:4px;width:231px;margin:0px auto;">\
                                  <h5 style="color:#666666;font-size: 16px;line-height: 22px;">推荐人姓名：<span class="parent-name"></span></h5>\
                                  <h5 style="color:#666666;font-size: 16px;line-height: 22px;">推荐人电话：<span class="parent-mobile"></span></h5>\
                                </div>\
                                <h6 style="font-size:12px;line-height: 17px;color:#666666;">推荐人将成为您的服务导师，助您快速创业，请谨慎填写，一旦提交不支持修改!</h6>\
                              </div>\
                            </div>\
                            <div style="display:flex;flex-direction: row;justify-content:center;">\
                              <div class="cancel-btn" style="flex:1;text-align: center;background:#FFFFFF ;padding:10px;border-top:1px solid #EEEEEE;border-radius:0 0 0 8px;color:#2979FF;font-size:17px">取消</div>\
                              <div class="ok-btn" style="flex:1;text-align: center;background:#2979FF;padding:10px; border-radius:0 0 8px 0;color:#FFFFFF;font-size:17px">确定</div>\
                            </div>\
                          </div>';
    /** 变量声明 **/
    $scope.localStorage=window.localStorage;
    $scope.promotionEditable = true;
  //  $scope.addressName = '';//地区名称
    $scope.addressName ='请选择';
    $scope.goToAddress = false;//判断是否进入到地区列表
    $scope.categoryData = [];//联盟分类数据
    $scope.selectText = '请选择';//联盟分类选择文本
      ...
      if ($scope.level > -2) {//xyz修改2级本地获取
        $http.get("data/region.json")
          .success(function (response) {
            ...
            if($scope.level == -1){
              ...
              $scope.applyInfo.num=$scope.nowLevel;
              console.log($scope.applyInfo.num)
              //重选市
       ...
       console.log($scope.addressName)
        ...
        ADDADDRESSService.getLocationList(item.value,ah).success(function (response) {
          ...
          console.log($scope.dataAddStreet);
          if(ah==2){
            areaValue=item.value;
          }
          var defaultValueFy=JSON.stringify($scope.defaultValue);
...
//      $scope.$ionicGoBack($scope.level);
    ...
    $scope.checkChange = function (obj,index) {
      OpenStoreService.checkText(obj)
        .success(function (response) {
    ...
    var contentWatch = $scope.$watch("applyInfo", function() {
      if (
		  ...
		  (!$scope.addressName||$scope.addressName==null||$scope.addressName.length==0||$scope.addressName=='请选择')||
          (
            $scope.promotionCode.length == 0 &&
            $scope.applyInfo.promotionCode.length == 0
          )
      ) {
    ...
    $scope.goAuthentication=function () {
     ...
     //   $scope.showAlert = true;
        ...
        var promotionCode=$scope.promotionCode&&$scope.promotionCode.length>0?$scope.promotionCode:$scope.applyInfo.promotionCode;
        OpenStoreService.getPromotionUserInfo($scope.promotionCode&&$scope.promotionCode.length>0?$scope.promotionCode:$scope.applyInfo.promotionCode)
          .success(function(response){
            if(response.success){
              if(!response.data||response.data==null){
                PopupService.showToast("推荐码不存在！");
                return;
              }
              var dialog=PopupService.dialog({
                onOpen:function(){
                  this.find(".promotion-code").html(response.data.promotionCode);
                  this.find(".parent-name").html(response.data.realName);
                  this.find(".parent-mobile").html(response.data.mobile);
                },
                content:parentInfoView,
                buttons:[
                  {selector:".ok-btn",handler:function(){
                    ...
                    // OpenStoreService.checkPerfectInformation($scope.applyInfo.storeName,$scope.selectValue,$scope.xyzPromotionCode,$scope.applyInfo.hrCode,$scope.adrProvinceId,$scope.adrCityId,$scope.adrAreaId,$scope.adrStreetId,$scope.addressName,$scope.applyInfo.address)
                    //  .success(function (response) {
                    ...
                    // 新开店接口
                    if($scope.memberId){ //我要开店
                      OpenStoreService.applySubmitStore($scope.memberId,$scope.applyInfo.storeName,$scope.selectValue,
                      ...
                      OpenStoreService.newShop($scope.applyInfo.storeName,$scope.selectValue,
                        promotion,$scope.applyInfo.hrCode,$scope.adrProvinceId,
                        $scope.adrCityId,$scope.adrAreaId,$scope.addressName,
              ...
              PopupService.showToast("获取推荐人信息失败！");
            }
          });
    ...
    $scope.getPromotionCode = function(){
      ...
      if (!$scope.adrAreaId && $scope.addressName=='请选择' && !$scope.adrCityId && !$scope.adrProvinceId){
        ...
        if($scope.promotionCode.length!=0){
          ...
          OpenStoreService.getPromotion($scope.adrAreaId)
            .success(function (res) {
              if(res.success){
    ...
    $scope.$on('$ionicView.beforeEnter', function (e,v) {
      $scope.promotionEditable=$stateParams.promotionCode && $stateParams.promotionCode!=null && $stateParams.promotionCode!='';
...
APP.service('OpenStoreService', ['$http', 'UrlService','$window', function ($http, UrlService,$window) {
  /*****
   * 获取推荐人信息
   * @param promotionCode
   * @returns {*}
   */
  this.getPromotionUserInfo = function(promotionCode){
    return $http({
      method: 'GET',
      url: UrlService.getUrl('GET_PROMOTION_USER_INFO')+"?promotionCode="+promotionCode
    });
  }
  //申请开店
